...
template <>
InputParameters
validParams<DGTotFluxContPotDriven>()
{
    ...
    _mu(getMaterialProperty
{
  ...
  {
    mooseError("In order to use this kernel, you must specify a boundary where it will live.");
    std::exit(1);
  }
}
...
DGTotFluxContPotDriven::computeQpResidual(Moose::DGResidualType type)
{
  ...
  const unsigned int elem_b_order =
      static_cast<unsigned int>(is_elem ? _var.order() : _neighbor_var.order());
  const double h_elem =
  ...
  {
    case Moose::Element:
      r += 0.5 * (-_D * _grad_u[_qp] * _normals[_qp] * _test[_i][_qp] +
                  _epsilon * _grad_test[_i][_qp] * _normals[_qp] * _D * _u[_qp]);
      r += _sigma / h_elem * _u[_qp] * _test[_i][_qp];

      r += 0.5 *
           (-_D_neighbor * _grad_neighbor_value[_qp] * _normals[_qp] * _test[_i][_qp] -
            _epsilon * _grad_test[_i][_qp] * _normals[_qp] * _D_neighbor * _neighbor_value[_qp]);
      r += -_sigma / h_elem * _neighbor_value[_qp] * _test[_i][_qp];
      break;

    case Moose::Neighbor:
      r += 0.5 * (_D * _grad_u[_qp] * _normals[_qp] +
                  _D_neighbor * _grad_neighbor_value[_qp] * _normals[_qp]) *
           _test_neighbor[_i][_qp];
      r += _epsilon * 0.5 * _grad_test_neighbor[_i][_qp] * _normals[_qp] *
           (_D * _u[_qp] - _D_neighbor * _neighbor_value[_qp]);
...
DGTotFluxContPotDriven::computeQpJacobian(Moose::DGJacobianType type)
{
  ...
  switch (type)
  {
    ...
    case Moose::NeighborNeighbor:
      ...
      r -= _epsilon * 0.5 * _grad_test_neighbor[_i][_qp] * _normals[_qp] * _D_neighbor *
