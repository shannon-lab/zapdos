...
FaceFaceConstraint::reinit()
{
  ...
  // _normals_master.resize(nqp);
  // _normals_slave.resize(nqp);
  ...
  for (_qp = 0; _qp < nqp; _qp++)
  {
    ...
    if (master_pinfo && slave_pinfo)
    {
      ...
      // _grad_u_master = _master_var.gradSln();
      // _normals_master[_qp] = _master_var.normals();
      ...
      // _grad_u_slave[_qp] = _slave_var.gradSln();
      // _normals_slave[_qp] = _slave_var.normals();
