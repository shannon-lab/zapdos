...
HagelaarElectronBC::computeQpResidual()
{
  ...
  return _test[_i][_qp] * _r_units * (1. - _r) / (1. + _r) * (-(2 * _a - 1) * _muem[_qp] * -_grad_potential[_qp] * _r_units * std::exp(_u[_qp]) * _normals[_qp] + 0.5 * _v_thermal * std::exp(_u[_qp]) - 0.5 * _v_thermal * _n_gamma) - _test[_i][_qp] * _r_units * 2. / (1. + _r) * (1. - _a) * _se_coeff[_qp] * _ion_flux * _normals[_qp];
}
...
HagelaarElectronBC::computeQpOffDiagJacobian(unsigned int jvar)
{
  ...
  return _test[_i][_qp] * _r_units * (1. - _r) / (1. + _r) * (-(2 * _a - 1) * _muem[_qp] * -_grad_phi[_j][_qp] * _r_units * std::exp(_u[_qp]) * _normals[_qp] - 0.5 * _v_thermal * _d_n_gamma_d_potential) - _test[_i][_qp] * _r_units * 2. / (1. + _r) * (1. - _a) * _se_coeff[_qp] * _d_ion_flux_d_potential * _normals[_qp];
  }
  ...
  else if (jvar == _mean_en_id)
  {
    ...
    return _test[_i][_qp] * _r_units * (1. - _r) / (1. + _r) * (-(2 * _a - 1) * _d_muem_d_actual_mean_en[_qp] * _actual_mean_en * _phi[_j][_qp] * -_grad_potential[_qp] * _r_units * std::exp(_u[_qp]) * _normals[_qp] + 0.5 * _d_v_thermal_d_mean_en * std::exp(_u[_qp]) - 0.5 * _d_v_thermal_d_mean_en * _n_gamma - 0.5 * _v_thermal * _d_n_gamma_d_mean_en);
  }
