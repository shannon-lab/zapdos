...
Real ElectronAdvectionDoNothingBC::computeQpResidual()
{
  return _muem[_qp] * _sign[_qp] * std::exp(_u[_qp]) * -_grad_potential[_qp] * _position_units * _normals[_qp] * _test[_i][_qp]  * _position_units;
}

Real ElectronAdvectionDoNothingBC::computeQpJacobian()
{
  ...
  return (_d_muem_d_u * _sign[_qp] * std::exp(_u[_qp]) * -_grad_potential[_qp] * _position_units + _muem[_qp] * _sign[_qp] * std::exp(_u[_qp]) * _phi[_j][_qp] * -_grad_potential[_qp] * _position_units) * _normals[_qp] * _test[_i][_qp] * _position_units;
}

Real ElectronAdvectionDoNothingBC::computeQpOffDiagJacobian(unsigned int jvar)
{
  if (jvar == _potential_id)
  {
    return _muem[_qp] * _sign[_qp] * std::exp(_u[_qp]) * -_grad_phi[_j][_qp] * _position_units * _normals[_qp] * _test[_i][_qp] * _position_units;
  }
  ...
  return _d_muem_d_mean_en * _sign[_qp] * std::exp(_u[_qp]) * -_grad_potential[_qp] * _position_units * _normals[_qp] * _test[_i][_qp] * _position_units;
  }
