#include "HagelaarElectronLFA_BC.h"
...
HagelaarElectronLFA_BC::HagelaarElectronLFA_BC(const InputParameters & parameters) :
...
Real
HagelaarElectronLFA_BC::computeQpResidual()
{
...
Real
HagelaarElectronLFA_BC::computeQpJacobian()
{
  if ( _normals[_qp] * -1.0 * -_grad_potential[_qp] > 0.0) {
    _a = 1.0;
  }
...
Real
HagelaarElectronLFA_BC::computeQpOffDiagJacobian(unsigned int jvar)
{
