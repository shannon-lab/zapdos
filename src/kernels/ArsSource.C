...
ArsSource::computeQpOffDiagJacobian(unsigned int jvar)
{
  ...
  if (jvar == _em_id)
    {
      return -_test[_i][_qp]*(_k2*_Ar[_qp]*_phi[_j][_qp]-_k3*_phi[_j][_qp]*_u[_qp]-_k5*_phi[_j][_qp]*_u[_qp]+_Ar[_qp]*_em[_qp]*_dk2_dTe*_dTe_d_em*_phi[_j][_qp]-_em[_qp]*_u[_qp]*_dk3_dTe*_dTe_d_em*_phi[_j][_qp]-_k5*_em[_qp]*_u[_qp]*_dk5_dTe*_dTe_d_em*_phi[_j][_qp]);
    }
  else if (jvar == _mean_electron_energy_id)
    {
      return -_test[_i][_qp]*(_Ar[_qp]*_em[_qp]*_dk2_dTe*_dTe_d_mean_el_energy*_phi[_j][_qp]-_em[_qp]*_u[_qp]*_dk3_dTe*_dTe_d_mean_el_energy*_phi[_j][_qp]-_k5*_em[_qp]*_u[_qp]*_dk5_dTe*_dTe_d_mean_el_energy*_phi[_j][_qp]);
    }
  else
    {
      return 0.0;
