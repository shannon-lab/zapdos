...
ElectronEnergyTermElasticRate::ElectronEnergyTermElasticRate(const InputParameters & parameters)
  : Kernel(parameters),
    ...
    _d_iz_d_actual_mean_en(
        getMaterialProperty<Real>("d_k_d_en_" + getParam<std::string>("reaction"))),
...
ElectronEnergyTermElasticRate::computeQpJacobian()
{
  Real energy_elastic = -3.0 * (_massIncident[_qp] / _massTarget[_qp]) * 2.0 / 3.0 *
                        std::exp(_u[_qp] - _electron[_qp]);
  Real d_elastic_d_en = -3.0 * (_massIncident[_qp] / _massTarget[_qp]) * 2.0 / 3.0 *
                        std::exp(_u[_qp] - _electron[_qp]) * _phi[_j][_qp];

  // return -_test[_i][_qp] * d_iz_d_mean_en * std::exp(_v[_qp]) * std::exp(_em[_qp]) *
  // _energy_change;
  return -_test[_i][_qp] * std::exp(_target[_qp] + _electron[_qp]) *
         (_rate_coefficient[_qp] * d_elastic_d_en + _d_iz_d_actual_mean_en[_qp] * energy_elastic);
}
