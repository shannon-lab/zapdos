...
DriftDiffusionElectrons::computeQpResidual()
{
        return _muem[_qp] * _sign[_qp] * std::exp(_u[_qp]) * -_grad_potential[_qp] * _r_units * -_grad_test[_i][_qp] * _r_units
                        - _diffem[_qp] * std::exp(_u[_qp]) * _grad_u[_qp] * _r_units * -_grad_test[_i][_qp] * _r_units;
}
...
DriftDiffusionElectrons::computeQpJacobian()
{
        ...
        return (_d_muem_d_u * _sign[_qp] * std::exp(_u[_qp]) * -_grad_potential[_qp] * _r_units + _muem[_qp] * _sign[_qp] * std::exp(_u[_qp]) * _phi[_j][_qp] * -_grad_potential[_qp] * _r_units) * -_grad_test[_i][_qp] * _r_units
                        - _diffem[_qp] * (std::exp(_u[_qp]) * _grad_phi[_j][_qp] * _r_units + std::exp(_u[_qp]) * _phi[_j][_qp] * _grad_u[_qp] * _r_units) * -_grad_test[_i][_qp] * _r_units - _d_diffem_d_u * std::exp(_u[_qp]) * _grad_u[_qp] * _r_units * -_grad_test[_i][_qp] * _r_units;
}
...
DriftDiffusionElectrons::computeQpOffDiagJacobian(unsigned int jvar)
{
        if (jvar == _potential_id) {
                ...
                return _d_muem_d_mean_en * _sign[_qp] * std::exp(_u[_qp]) * -_grad_potential[_qp] * _r_units * -_grad_test[_i][_qp] * _r_units
                                - _d_diffem_d_mean_en * std::exp(_u[_qp]) * _grad_u[_qp] * _r_units * -_grad_test[_i][_qp] * _r_units;
        } else {
                return 0.;
        }
}
