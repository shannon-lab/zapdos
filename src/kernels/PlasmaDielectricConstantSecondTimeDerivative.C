...
PlasmaDielectricConstantSecondTimeDerivative::computeQpResidual()
{
  ...
  if (_component == REAL)
  {
    term = _eps_r_real_dot_dot[_qp] * _real_value[_qp] -
           _eps_r_imag_dot_dot[_qp] * _imag_value[_qp] + _eps_r_real[_qp] * _real_dot_dot[_qp] -
           _eps_r_imag[_qp] * _imag_dot_dot[_qp];
  }
  else if (_component == IMAGINARY)
  {
    term = _eps_r_real_dot_dot[_qp] * _imag_value[_qp] +
           _eps_r_imag_dot_dot[_qp] * _real_value[_qp] + _eps_r_real[_qp] * _imag_dot_dot[_qp] +
           _eps_r_imag[_qp] * _real_dot_dot[_qp];
  }
