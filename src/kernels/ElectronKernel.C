...
ElectronKernel::computeQpResidual()
{
  ...
  return -_grad_test[_i][_qp]*std::exp(_u[_qp])*(-_muem*-_grad_potential[_qp]-_diff*_grad_u[_qp]) // Transport
    ...
    -_test[_i][_qp]*0.35*std::exp(-1.65e7/_grad_potential.size())*(-_muem*-_grad_potential[_qp]*std::exp(_u[_qp])-_diff*std::exp(_u[_qp])*_grad_u[_qp]).size() // Reaction. Townsend coefficient formulation
...
ElectronKernel::computeQpJacobian()
{
  ...
  return -_grad_test[_i][_qp]*std::exp(_u[_qp])*(-_muem*-_grad_potential[_qp]*_phi[_j][_qp] - _diff*(_phi[_j][_qp]*_grad_u[_qp]+_grad_phi[_j][_qp]))
    ...
    -_test[_i][_qp]*0.35*std::exp(-1.65e7/_grad_potential.size())*_flux*_d_flux_d_u/std::sqrt(_flux*_flux) // Reaction. Townsend coefficient formulation
