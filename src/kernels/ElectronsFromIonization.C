...
ElectronsFromIonization::computeQpJacobian()
{
  ...
  RealVectorValue d_electron_flux_d_em = -_muem[_qp]*-_grad_potential[_qp]*std::exp(_u[_qp])*_phi[_j][_qp] - _diffem[_qp]*std::exp(_u[_qp])*_phi[_j][_qp]*_grad_u[_qp]-_diffem[_qp]*std::exp(_u[_qp])*_grad_phi[_j][_qp];
...
ElectronsFromIonization::computeQpOffDiagJacobian(unsigned int jvar)
{
  ...
  Real d_iz_term_d_mean_en = (electron_flux_mag * d_iz_d_mean_en);
